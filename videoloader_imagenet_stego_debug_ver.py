...
def get_yaml(dir,name):
    dir_grays = osp.join(dir,name) # where is the YAML file produced? Generate YAML file myself????
    print('dir_grays: ', dir_grays)
...
def parse_images(dir):
    ...
    print('gray_dict:', gray_dict)
    print('small_dict:', small_dict)
    print('error_dict:', error_dict)
    print('mono_dict:', mono_dict)
    image_pairs = []
    bad_num = 0
    # How does this analogies section generate image pairs?
    # create new logic without npy????
    ...
    for i in range(len(analogies)):
        ...
        #  what is mmap mode?
        analogy_path = analogy_path.split("analogies\\")[1] #IndexError: list index out of range #("analogies/")[1]
        #print("yamls:",mono_dict,"analogy_path",analogy_path)

        # KeyError: 'analogies_new.npy'
        print(gray_dict[analogy_path])
        bad_list = list(set(gray_dict[analogy_path]))
        # bad_listOG = list(set(gray_dict[analogy_path]+small_dict[analogy_path]+mono_dict[analogy_path]+error_dict[analogy_path]))
        # HOW DO I CREATE A PAIR WITH THIS LOGIC?????!
...
parse_images(".\dataset\coco-2017")
...
class VideosDataset_ImageNet(data.Dataset):
    def __init__(
        ...
        image_pairs = []
        curr_image_pairs = parse_images(data_root) # yaml file error here
