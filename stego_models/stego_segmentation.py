...
class STEGO_seg():
    def __init__(self,cra=False,n_clusters=27):
        ...
        '''
        model = LitUnsupervisedSegmenter.load_from_checkpoint("/content/FYP_recoloration/dataset/checkpoints/spcolor/checkpoints/cocostuff27_vit_base_5.ckpt", map_location='cpu') # for colab
    ...
    def got_plot(self,cluster_preds,nrow = 4,cluster_value=None):
       ...
       # True if cluster_value is not None else False test
        print('cluster_value: ', cluster_value)
        #FIX torch.Size([8, 3, 32, 32]) @ torch.Size([8, 3, 256, 256])
        # Dim 3: 32 != 256
        if cluster_value is not None:
            if cluster_value.ndim == 3:
                cluster_value = cluster_value.unsqueeze(1)
            # if cluster_value.ndim == 4:
            #     cluster_value = cluster_value.unsqueeze(1)
            # if cluster_value.dim() == 3:
            #     cluster_value = cluster_value.unsqueeze(1)  # Adds a singleton dimension for channels
            # if cluster_value.shape[1] == 1: # uncommet and replace for upsample
            #     # Repeat cluster_value across the channel dimension to match pred_class's channels
            #     cluster_value = cluster_value.expand(-1, 3, -1, -1)
            pred_class_upsampled  = F.interpolate(pred_class, size=(256, 256), mode='bilinear', align_corners=False)
            pred_class = pred_class_upsampled * cluster_value 
        
        if cluster_value is None:
            pred_class = pred_class
            
            # pred_class = pred_class * cluster_value OG
            # pred_class = pred_class * cluster_value
            #The size of tensor a (32) must match the size of tensor b (256) at non-singleton dimension 3
        #print("plot_cluster",plot_cluster.shape)
...
if __name__ == "__main__":
    ...
    # TODO: fix save result dir 
    os.makedirs(result_dir,exist_ok=True)

    #plot_class_colors(stego)

    for i, img in enumerate(tqdm(loader)):
        # cluster_value,cluster_preds = stego.my_app(img.cuda()) # remove cuda
        cluster_value,cluster_preds = stego.my_app(img) # remove cuda
