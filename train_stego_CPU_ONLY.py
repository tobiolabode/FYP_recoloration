...
from models.NonlocalNet_stego_CPU import (NonlocalWeightedAverage, VGG19_pytorch,
...
# parser.add_argument("--data_root_imagenet", default="/dataset/ImageNet_train/", type=str) OG code
...
parser.add_argument("--resume_iter", type=int, default=76000) #OG 120000
...
#     print('gpu_setup_DDP-device:', device)
#     return device

def cpu_setup():
    device = torch.device('cpu')
    print('cpu_setup device: ', device)
    return device
...
def load_data_imagenet():
    ...
    # train_sampler = torch.RandomSampler(train_dataset_imagenet)
...
def key_transformation(old_key):
        # if old_key == "layer.0.weight":
        #     return "layer.1.weight"
        # if old_key == "theta_class.weight":
        #     return "theta.weight"
        # if old_key == "theta_class.bias":
        #     return "theta.bias"
        # if old_key == "theta_class.bias":
        #     return "theta.bias"
        # if old_key == "phi_class.weight":
        #     return "phi.weight"
        # if old_key == "phi_class.bias":
        #     return "phi.bias"

        if old_key == "layer.0.weight":
            return "layer.1.weight"
        
        if old_key == "theta.weight":
            return "theta_class.weight"
        if old_key == "theta.bias":
            return "theta_class.bias"
        
        if old_key == "phi.weight":
            return "phi_class.weight"
        if old_key == "phi.bias":
            return "phi_class.bias"

        return old_key



def colornet_key_transformation(old_key):

        if old_key == "conv1_1.0.weight":
            return "conv1_1_new.0.weight"
        
        if old_key == "conv1_1.0.bia":
            return "conv1_1.0.bias"
...
def resume_model():									   #继续学习 ，加载参数
    ...
    if opt.resume_iter:
        ...
        nonlocal_net_temp = WarpNet(8) 
        loaded_state_dict = torch.load(new_checkpoint, map_location=torch.device('cpu')) # 256, 256, 1,1 
        current_model_dict = nonlocal_net_temp.state_dict()
        new_state_dict={k:v if v.size()==current_model_dict[k].size()  else  current_model_dict[k] for k,v in zip(current_model_dict.keys(), loaded_state_dict.values())}
        nonlocal_net.load_state_dict(new_state_dict, strict=False)
        ...
        # new_checkpoint = torch.load(new_checkpoint)
        print('new_checkpoint', new_checkpoint)

        #track vars / tensors
        # new_state_dict['phi_class.weight'].shape
        # loaded_state_dict['phi_class.weight'].shape = torch.Size(256,256, 1, 1)
        # mismatch here

        # TypeError: Expected state_dict to be dict-like, got <class 'str'>.
        # nonlocal_net.module.load_state_dict(torch.load(new_checkpoint)) # error here:  RuntimeError: Error(s) in loading state_dict for WarpNet:
        # nonlocal_net.load_state_dict(torch.load(new_checkpoint), strict=True) # error here:  RuntimeError: Error(s) in loading state_dict for WarpNet:
                                                        # Missing key(s) in state_dict: "theta_class.weight", "theta_class.bias", "phi_class.weight", "phi_class.bias".
                                                        # Unexpected key(s) in state_dict: "theta.weight", "theta.bias", "phi.weight", "phi.bias".
       
        #TODO:
        #respahe state dict arrays. 256, 256, 1, 1 vs 256, 256!!!
        # if not ciruclar CUDA mistach error.
        
        # RuntimeError: Error(s) in loading state_dict for WarpNet:
	    # size mismatch for theta_class.weight: copying a param with shape torch.Size([256, 256, 1, 1]) from checkpoint, the shape in current model is torch.Size([256, 256]).
        ...
        target_colornet_path = f"{opt.checkpoint_dir}colornet_iter_{total_iter}_newkeys.pth"
        new_checkpoint_colornet = target_colornet_path
       
        temp_colournet_path = os.path.join(opt.checkpoint_dir, "colornet_iter_%d.pth" % total_iter)
        
        # checkpoint = torch.load(os.path.join(opt.checkpoint_dir, "colornet_iter_%d.pth" % total_iter),map_location='cpu')
       
        rename_state_dict_keys(source=temp_colournet_path, key_transformation=colornet_key_transformation, target=new_checkpoint_colornet) 
        ...
        # fix mismatch code here 

        colornet_temp = ColorVidNet(4)  # opt.ic
        ...
        loaded_state_dict = torch.load(new_checkpoint_colornet, map_location=torch.device('cpu')) # 256, 256, 1,1 
        current_model_dict = colornet_temp.state_dict()
        new_state_dict={k:v if v.size()==current_model_dict[k].size()  else  current_model_dict[k] for k,v in zip(current_model_dict.keys(), loaded_state_dict.values())}
        colornet.load_state_dict(new_state_dict, strict=False) 


        # colornet.load_state_dict(torch.load(new_checkpoint_colornet)) #checkpoint



        # discriminator remap
        discriminator_temp = Discriminator_x64(in_size=3,ndf=64)
        ...
        current_model_dict = discriminator_temp.state_dict()
        new_state_dict={k:v if v.size()==current_model_dict[k].size()  else  current_model_dict[k] for k,v in zip(current_model_dict.keys(), loaded_state_dict.values())}
...
def to_device_CPU(
    ...
    print('to_device_CPU: ', device)
    # add assert statement forcing CPU-mode!
...
def to_device(
    ...
    nonlocal_net = torch.nn.DataParallel(nonlocal_net.to(device), device_ids=opt.gpu_ids) # running this model
...
#     # colornet = torch.nn.parallel.DistributedDataParallel(colornet.to(device), device_ids=[local_rank])
#     # nonlocal_net = torch.nn.parallel.DistributedDataParallel(nonlocal_net.to(device), device_ids=[local_rank])
#     # discriminator = torch.nn.parallel.DistributedDataParallel(discriminator.to(device), device_ids=[local_rank])
    
#     # non CPU version:
#     colornet = colornet.to(device)
#     nonlocal_net = nonlocal_net.to(device)
#     discriminator = discriminator.to(device)
#     #vggnet = torch.nn.parallel.DistributedDataParallel(vggnet.to(device), device_ids=[local_rank])
#     #contextual_loss = torch.nn.parallel.DistributedDataParallel(contextual_loss.to(device), device_ids=[local_rank])
#     #contextual_forward_loss = torch.nn.parallel.DistributedDataParallel(contextual_forward_loss.to(device), device_ids=[local_rank])
#     #weighted_layer_color = torch.nn.parallel.DistributedDataParallel(weighted_layer_color.to(device), device_ids=[local_rank])
#     #nonlocal_weighted_layer = torch.nn.parallel.DistributedDataParallel(nonlocal_weighted_layer.to(device), device_ids=[local_rank])
#     #instancenorm = torch.nn.parallel.DistributedDataParallel(instancenorm.to(device), device_ids=[local_rank])
#     #downsampling_by2 = torch.nn.parallel.DistributedDataParallel(downsampling_by2.to(device), device_ids=[local_rank])
#     vggnet = vggnet.to(device)
#     contextual_loss = contextual_loss.to(device)
#     contextual_forward_loss = contextual_forward_loss.to(device)
#     weighted_layer_color = weighted_layer_color.to(device)
