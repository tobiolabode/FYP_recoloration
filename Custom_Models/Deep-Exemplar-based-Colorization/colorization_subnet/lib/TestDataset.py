...
def combo5_loader(path, real_w, real_h):
	print(f'combo5_loader path: {path}')
	f = open(path, 'rb')
...
class TestDataset(data.Dataset):
	...
	def __getitem__(self, index):
		...
		image_comb_name = "%s_%s" % (os.path.splitext(image_names[0])[0], os.path.splitext(image_names[1])[0])
		combo_path = osp.join(self.data_root, "combo_new", "%s.combo" % image_comb_name)
		print(f'image_comb_name: {image_comb_name}')
		print(f'combo_path: {combo_path}')
		errs, warp_ba, warp_aba = combo5_loader(combo_path, w, h)
		# in3_ref3.combo out of index or list / file does not exist
